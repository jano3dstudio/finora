...
class DesktopWindow:JanoWindow {
 ...
 public DesktopWindow(){Text="FINORA";AutoScaleMode=AutoScaleMode.None;BackColor=Tokens.Background;var area=Screen.PrimaryScreen.WorkingArea;ClientSize=new Size(Math.Min(1440,area.Width-80),Math.Min(920,area.Height-110));MinimumSize=new Size(800,600);StartPosition=FormStartPosition.CenterScreen;
 web.Dock=DockStyle.Fill;web.DefaultBackgroundColor=Tokens.Background;Controls.Add(web);loading=new Label{Dock=DockStyle.Fill,Text="FINORA …",ForeColor=Tokens.Text,TextAlign=ContentAlignment.MiddleCenter};Controls.Add(loading);loading.BringToFront();Shown+=async(s,e)=>await Initialize();
